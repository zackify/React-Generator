...
if(type == "component"){
  fs.readFile(`${templatePath}/components/es6-component.js`, 'utf8', function (err,data) {
    ...
    if(settings.constructor) result = result.replace(/CONSTRUCTOR/g, fs.readFileSync(`${templatePath}/methods/constructor.js`));
    else result = result.replace(/CONSTRUCTOR/g, '');


    fs.writeFile(`${buildFolder}/${componentName}.${settings.extension}`, result, 'utf8', function (err) {
       if (err) return console.log(err);
       else console.log(`Successfully Generated ${componentName}`)
    });
  });
}